import numpy as np
import pandas as pd
import random as rd
from otree.api import (
    models, widgets, BaseConstants, BaseSubsession, BaseGroup, BasePlayer,
    Currency as c, currency_range
)

def loadLotteries(url: str, sheet: str):
    f = pd.read_excel(url, sheet_name=f'{sheet}')
    d = {
        'type': []
    }
    cols = f.columns.tolist()
    for col in cols:
        d[col] = []
        for row in np.arange(0, len(f), 1):
            d[col].append(f[col].loc[row])
    for row in np.arange(0, len(f), 1):
        d['type'].append(sheet)
    return d


gain_data = loadLotteries('_static/data/lotteries.xls', 'GAIN')
loss_data = loadLotteries('_static/data/lotteries.xls', 'LOSS')


def separateLotteries(d: dict) -> list[dict]:
    d1 = dict(d)
    d2 = dict(d)
    d1['asc'] = []
    d2['asc'] = []
    for i in np.arange(0, len(d[list(d.keys())[0]]), 1):
        randomizer = rd.choice([True, False])
        if randomizer is True:
            d1['asc'].append(randomizer)
            d2['asc'].append(not randomizer)
        if randomizer is False:
            d1['asc'].append(randomizer)
            d2['asc'].append(not randomizer)
    return [d1, d2]


gain_data = separateLotteries(gain_data)
loss_data = separateLotteries(loss_data)


def getPayoffOptionB(block: int, table: int, decision: int, typ: str):
    data = None
    if typ is str('GAIN'):
        data = gain_data
    if typ is str('LOSS'):
        data = loss_data
    li = []
    b_start = data[block-1]['x1'][table-1]
    b_step = (-1)*(data[block-1]['x1'][table-1] - data[block-1]['x2'][table-1])/20
    b_stop = data[block-1]['x2'][table-1] + b_step
    for i in np.arange(b_start, b_stop, b_step):
        li.append(i)
    if data[block-1]['asc'][table-1] is True:
        li.reverse()
    return li[decision-1]


def getPayoffOptionA(block: int, table: int, typ: str):
    data = None
    if typ is str('GAIN'):
        data = gain_data
    if typ is str('LOSS'):
        data = loss_data
    return {
        'p': data[block-1]['p'][table-1],
        'x1': data[block-1]['x1'][table-1],
        'x2': data[block-1]['x2'][table-1]
    }


class Constants(BaseConstants):
    name_in_url = 'Game'
    players_per_group = None
    num_rounds = 1

    multiplier = 15
    basic_fee = 15
    gain_tables_per_block = len(gain_data[0][list(gain_data[0].keys())[0]])
    loss_tables_per_block = len(loss_data[0][list(loss_data[0].keys())[0]])
    tables_per_block = gain_tables_per_block + loss_tables_per_block
    blocks = 2
    tables = blocks * tables_per_block
    decisions_per_table = 21


# Block 1 Gain 1
B1_GAIN1_D1 = 2
B1_GAIN1_D2 = 2
B1_GAIN1_D3 = 2
B1_GAIN1_D4 = 2
B1_GAIN1_D5 = 1
B1_GAIN1_D6 = 2
B1_GAIN1_D7 = 1
B1_GAIN1_D8 = 1
B1_GAIN1_D9 = 1
B1_GAIN1_D10 = 2
B1_GAIN1_D11 = 1
B1_GAIN1_D12 = 2
B1_GAIN1_D13 = 2
B1_GAIN1_D14 = 1
B1_GAIN1_D15 = 2
B1_GAIN1_D16 = 2
B1_GAIN1_D17 = 2
B1_GAIN1_D18 = 1
B1_GAIN1_D19 = 1
B1_GAIN1_D20 = 1
B1_GAIN1_D21 = 2

# Block 1 Gain 2
B1_GAIN2_D1 = 1
B1_GAIN2_D2 = 2
B1_GAIN2_D3 = 2
B1_GAIN2_D4 = 1
B1_GAIN2_D5 = 2
B1_GAIN2_D6 = 2
B1_GAIN2_D7 = 1
B1_GAIN2_D8 = 2
B1_GAIN2_D9 = 1
B1_GAIN2_D10 = 1
B1_GAIN2_D11 = 2
B1_GAIN2_D12 = 1
B1_GAIN2_D13 = 1
B1_GAIN2_D14 = 2
B1_GAIN2_D15 = 2
B1_GAIN2_D16 = 2
B1_GAIN2_D17 = 1
B1_GAIN2_D18 = 2
B1_GAIN2_D19 = 2
B1_GAIN2_D20 = 1
B1_GAIN2_D21 = 2

# Block 1 Gain 3
B1_GAIN3_D1 = 2
B1_GAIN3_D2 = 2
B1_GAIN3_D3 = 2
B1_GAIN3_D4 = 2
B1_GAIN3_D5 = 1
B1_GAIN3_D6 = 1
B1_GAIN3_D7 = 1
B1_GAIN3_D8 = 1
B1_GAIN3_D9 = 2
B1_GAIN3_D10 = 1
B1_GAIN3_D11 = 1
B1_GAIN3_D12 = 1
B1_GAIN3_D13 = 1
B1_GAIN3_D14 = 2
B1_GAIN3_D15 = 2
B1_GAIN3_D16 = 1
B1_GAIN3_D17 = 1
B1_GAIN3_D18 = 1
B1_GAIN3_D19 = 1
B1_GAIN3_D20 = 2
B1_GAIN3_D21 = 1

# Block 1 Gain 4
B1_GAIN4_D1 = 2
B1_GAIN4_D2 = 2
B1_GAIN4_D3 = 2
B1_GAIN4_D4 = 2
B1_GAIN4_D5 = 1
B1_GAIN4_D6 = 2
B1_GAIN4_D7 = 2
B1_GAIN4_D8 = 2
B1_GAIN4_D9 = 1
B1_GAIN4_D10 = 1
B1_GAIN4_D11 = 1
B1_GAIN4_D12 = 1
B1_GAIN4_D13 = 2
B1_GAIN4_D14 = 2
B1_GAIN4_D15 = 2
B1_GAIN4_D16 = 1
B1_GAIN4_D17 = 2
B1_GAIN4_D18 = 2
B1_GAIN4_D19 = 1
B1_GAIN4_D20 = 2
B1_GAIN4_D21 = 1

# Block 1 Gain 5
B1_GAIN5_D1 = 1
B1_GAIN5_D2 = 2
B1_GAIN5_D3 = 1
B1_GAIN5_D4 = 2
B1_GAIN5_D5 = 2
B1_GAIN5_D6 = 2
B1_GAIN5_D7 = 1
B1_GAIN5_D8 = 2
B1_GAIN5_D9 = 1
B1_GAIN5_D10 = 2
B1_GAIN5_D11 = 2
B1_GAIN5_D12 = 1
B1_GAIN5_D13 = 1
B1_GAIN5_D14 = 2
B1_GAIN5_D15 = 2
B1_GAIN5_D16 = 2
B1_GAIN5_D17 = 2
B1_GAIN5_D18 = 1
B1_GAIN5_D19 = 1
B1_GAIN5_D20 = 1
B1_GAIN5_D21 = 2

# Block 1 Gain 6
B1_GAIN6_D1 = 1
B1_GAIN6_D2 = 1
B1_GAIN6_D3 = 2
B1_GAIN6_D4 = 1
B1_GAIN6_D5 = 1
B1_GAIN6_D6 = 2
B1_GAIN6_D7 = 1
B1_GAIN6_D8 = 1
B1_GAIN6_D9 = 2
B1_GAIN6_D10 = 1
B1_GAIN6_D11 = 1
B1_GAIN6_D12 = 2
B1_GAIN6_D13 = 2
B1_GAIN6_D14 = 1
B1_GAIN6_D15 = 2
B1_GAIN6_D16 = 1
B1_GAIN6_D17 = 2
B1_GAIN6_D18 = 2
B1_GAIN6_D19 = 1
B1_GAIN6_D20 = 2
B1_GAIN6_D21 = 1

# Block 1 Gain 7
B1_GAIN7_D1 = 2
B1_GAIN7_D2 = 2
B1_GAIN7_D3 = 1
B1_GAIN7_D4 = 1
B1_GAIN7_D5 = 2
B1_GAIN7_D6 = 2
B1_GAIN7_D7 = 1
B1_GAIN7_D8 = 2
B1_GAIN7_D9 = 1
B1_GAIN7_D10 = 1
B1_GAIN7_D11 = 2
B1_GAIN7_D12 = 2
B1_GAIN7_D13 = 2
B1_GAIN7_D14 = 1
B1_GAIN7_D15 = 1
B1_GAIN7_D16 = 2
B1_GAIN7_D17 = 2
B1_GAIN7_D18 = 2
B1_GAIN7_D19 = 1
B1_GAIN7_D20 = 1
B1_GAIN7_D21 = 2

# Block 1 Gain 8
B1_GAIN8_D1 = 2
B1_GAIN8_D2 = 2
B1_GAIN8_D3 = 1
B1_GAIN8_D4 = 2
B1_GAIN8_D5 = 2
B1_GAIN8_D6 = 1
B1_GAIN8_D7 = 2
B1_GAIN8_D8 = 1
B1_GAIN8_D9 = 1
B1_GAIN8_D10 = 2
B1_GAIN8_D11 = 2
B1_GAIN8_D12 = 1
B1_GAIN8_D13 = 1
B1_GAIN8_D14 = 2
B1_GAIN8_D15 = 1
B1_GAIN8_D16 = 2
B1_GAIN8_D17 = 2
B1_GAIN8_D18 = 1
B1_GAIN8_D19 = 1
B1_GAIN8_D20 = 2
B1_GAIN8_D21 = 1

# Block 1 Gain 9
B1_GAIN9_D1 = 1
B1_GAIN9_D2 = 1
B1_GAIN9_D3 = 1
B1_GAIN9_D4 = 2
B1_GAIN9_D5 = 2
B1_GAIN9_D6 = 1
B1_GAIN9_D7 = 1
B1_GAIN9_D8 = 1
B1_GAIN9_D9 = 1
B1_GAIN9_D10 = 1
B1_GAIN9_D11 = 2
B1_GAIN9_D12 = 1
B1_GAIN9_D13 = 2
B1_GAIN9_D14 = 1
B1_GAIN9_D15 = 2
B1_GAIN9_D16 = 2
B1_GAIN9_D17 = 1
B1_GAIN9_D18 = 1
B1_GAIN9_D19 = 1
B1_GAIN9_D20 = 2
B1_GAIN9_D21 = 1

# Block 1 Gain 10
B1_GAIN10_D1 = 2
B1_GAIN10_D2 = 2
B1_GAIN10_D3 = 1
B1_GAIN10_D4 = 2
B1_GAIN10_D5 = 2
B1_GAIN10_D6 = 2
B1_GAIN10_D7 = 2
B1_GAIN10_D8 = 2
B1_GAIN10_D9 = 1
B1_GAIN10_D10 = 1
B1_GAIN10_D11 = 1
B1_GAIN10_D12 = 2
B1_GAIN10_D13 = 1
B1_GAIN10_D14 = 2
B1_GAIN10_D15 = 2
B1_GAIN10_D16 = 1
B1_GAIN10_D17 = 1
B1_GAIN10_D18 = 2
B1_GAIN10_D19 = 2
B1_GAIN10_D20 = 1
B1_GAIN10_D21 = 1

# Block 1 Gain 11
B1_GAIN11_D1 = 1
B1_GAIN11_D2 = 1
B1_GAIN11_D3 = 1
B1_GAIN11_D4 = 2
B1_GAIN11_D5 = 2
B1_GAIN11_D6 = 2
B1_GAIN11_D7 = 1
B1_GAIN11_D8 = 1
B1_GAIN11_D9 = 1
B1_GAIN11_D10 = 1
B1_GAIN11_D11 = 2
B1_GAIN11_D12 = 2
B1_GAIN11_D13 = 1
B1_GAIN11_D14 = 2
B1_GAIN11_D15 = 2
B1_GAIN11_D16 = 2
B1_GAIN11_D17 = 1
B1_GAIN11_D18 = 1
B1_GAIN11_D19 = 2
B1_GAIN11_D20 = 2
B1_GAIN11_D21 = 1

# Block 1 Gain 12
B1_GAIN12_D1 = 2
B1_GAIN12_D2 = 2
B1_GAIN12_D3 = 1
B1_GAIN12_D4 = 2
B1_GAIN12_D5 = 2
B1_GAIN12_D6 = 2
B1_GAIN12_D7 = 2
B1_GAIN12_D8 = 2
B1_GAIN12_D9 = 2
B1_GAIN12_D10 = 1
B1_GAIN12_D11 = 1
B1_GAIN12_D12 = 1
B1_GAIN12_D13 = 1
B1_GAIN12_D14 = 2
B1_GAIN12_D15 = 1
B1_GAIN12_D16 = 1
B1_GAIN12_D17 = 2
B1_GAIN12_D18 = 2
B1_GAIN12_D19 = 1
B1_GAIN12_D20 = 2
B1_GAIN12_D21 = 2

# Block 1 Gain 13
B1_GAIN13_D1 = 1
B1_GAIN13_D2 = 1
B1_GAIN13_D3 = 2
B1_GAIN13_D4 = 1
B1_GAIN13_D5 = 2
B1_GAIN13_D6 = 1
B1_GAIN13_D7 = 2
B1_GAIN13_D8 = 1
B1_GAIN13_D9 = 2
B1_GAIN13_D10 = 1
B1_GAIN13_D11 = 2
B1_GAIN13_D12 = 2
B1_GAIN13_D13 = 1
B1_GAIN13_D14 = 2
B1_GAIN13_D15 = 2
B1_GAIN13_D16 = 2
B1_GAIN13_D17 = 1
B1_GAIN13_D18 = 2
B1_GAIN13_D19 = 1
B1_GAIN13_D20 = 2
B1_GAIN13_D21 = 2

# Block 1 Gain 14
B1_GAIN14_D1 = 2
B1_GAIN14_D2 = 2
B1_GAIN14_D3 = 1
B1_GAIN14_D4 = 1
B1_GAIN14_D5 = 2
B1_GAIN14_D6 = 1
B1_GAIN14_D7 = 1
B1_GAIN14_D8 = 1
B1_GAIN14_D9 = 1
B1_GAIN14_D10 = 1
B1_GAIN14_D11 = 2
B1_GAIN14_D12 = 1
B1_GAIN14_D13 = 2
B1_GAIN14_D14 = 1
B1_GAIN14_D15 = 2
B1_GAIN14_D16 = 2
B1_GAIN14_D17 = 2
B1_GAIN14_D18 = 2
B1_GAIN14_D19 = 2
B1_GAIN14_D20 = 1
B1_GAIN14_D21 = 2

# Block 1 Gain 15
B1_GAIN15_D1 = 2
B1_GAIN15_D2 = 1
B1_GAIN15_D3 = 2
B1_GAIN15_D4 = 2
B1_GAIN15_D5 = 1
B1_GAIN15_D6 = 1
B1_GAIN15_D7 = 1
B1_GAIN15_D8 = 1
B1_GAIN15_D9 = 1
B1_GAIN15_D10 = 1
B1_GAIN15_D11 = 1
B1_GAIN15_D12 = 1
B1_GAIN15_D13 = 1
B1_GAIN15_D14 = 1
B1_GAIN15_D15 = 2
B1_GAIN15_D16 = 1
B1_GAIN15_D17 = 2
B1_GAIN15_D18 = 1
B1_GAIN15_D19 = 1
B1_GAIN15_D20 = 1
B1_GAIN15_D21 = 2

# Block 1 Gain 16
B1_GAIN16_D1 = 1
B1_GAIN16_D2 = 2
B1_GAIN16_D3 = 1
B1_GAIN16_D4 = 2
B1_GAIN16_D5 = 1
B1_GAIN16_D6 = 2
B1_GAIN16_D7 = 1
B1_GAIN16_D8 = 1
B1_GAIN16_D9 = 1
B1_GAIN16_D10 = 1
B1_GAIN16_D11 = 2
B1_GAIN16_D12 = 1
B1_GAIN16_D13 = 1
B1_GAIN16_D14 = 1
B1_GAIN16_D15 = 2
B1_GAIN16_D16 = 2
B1_GAIN16_D17 = 1
B1_GAIN16_D18 = 2
B1_GAIN16_D19 = 2
B1_GAIN16_D20 = 2
B1_GAIN16_D21 = 2

# Block 1 Gain 17
B1_GAIN17_D1 = 1
B1_GAIN17_D2 = 1
B1_GAIN17_D3 = 1
B1_GAIN17_D4 = 2
B1_GAIN17_D5 = 2
B1_GAIN17_D6 = 2
B1_GAIN17_D7 = 2
B1_GAIN17_D8 = 1
B1_GAIN17_D9 = 1
B1_GAIN17_D10 = 1
B1_GAIN17_D11 = 2
B1_GAIN17_D12 = 1
B1_GAIN17_D13 = 2
B1_GAIN17_D14 = 2
B1_GAIN17_D15 = 2
B1_GAIN17_D16 = 1
B1_GAIN17_D17 = 2
B1_GAIN17_D18 = 2
B1_GAIN17_D19 = 1
B1_GAIN17_D20 = 1
B1_GAIN17_D21 = 2

# Block 1 Gain 18
B1_GAIN18_D1 = 1
B1_GAIN18_D2 = 1
B1_GAIN18_D3 = 2
B1_GAIN18_D4 = 2
B1_GAIN18_D5 = 1
B1_GAIN18_D6 = 2
B1_GAIN18_D7 = 1
B1_GAIN18_D8 = 2
B1_GAIN18_D9 = 2
B1_GAIN18_D10 = 2
B1_GAIN18_D11 = 2
B1_GAIN18_D12 = 2
B1_GAIN18_D13 = 2
B1_GAIN18_D14 = 2
B1_GAIN18_D15 = 1
B1_GAIN18_D16 = 1
B1_GAIN18_D17 = 1
B1_GAIN18_D18 = 2
B1_GAIN18_D19 = 2
B1_GAIN18_D20 = 1
B1_GAIN18_D21 = 2

# Block 1 Gain 19
B1_GAIN19_D1 = 2
B1_GAIN19_D2 = 2
B1_GAIN19_D3 = 2
B1_GAIN19_D4 = 1
B1_GAIN19_D5 = 1
B1_GAIN19_D6 = 1
B1_GAIN19_D7 = 1
B1_GAIN19_D8 = 2
B1_GAIN19_D9 = 1
B1_GAIN19_D10 = 1
B1_GAIN19_D11 = 1
B1_GAIN19_D12 = 2
B1_GAIN19_D13 = 2
B1_GAIN19_D14 = 2
B1_GAIN19_D15 = 2
B1_GAIN19_D16 = 2
B1_GAIN19_D17 = 2
B1_GAIN19_D18 = 2
B1_GAIN19_D19 = 2
B1_GAIN19_D20 = 1
B1_GAIN19_D21 = 2

# Block 1 Gain 20
B1_GAIN20_D1 = 1
B1_GAIN20_D2 = 2
B1_GAIN20_D3 = 1
B1_GAIN20_D4 = 2
B1_GAIN20_D5 = 1
B1_GAIN20_D6 = 1
B1_GAIN20_D7 = 2
B1_GAIN20_D8 = 2
B1_GAIN20_D9 = 1
B1_GAIN20_D10 = 1
B1_GAIN20_D11 = 1
B1_GAIN20_D12 = 1
B1_GAIN20_D13 = 1
B1_GAIN20_D14 = 1
B1_GAIN20_D15 = 1
B1_GAIN20_D16 = 2
B1_GAIN20_D17 = 2
B1_GAIN20_D18 = 2
B1_GAIN20_D19 = 1
B1_GAIN20_D20 = 1
B1_GAIN20_D21 = 2

# Block 1 Loss 1
B1_LOSS1_D1 = 2
B1_LOSS1_D2 = 2
B1_LOSS1_D3 = 2
B1_LOSS1_D4 = 1
B1_LOSS1_D5 = 2
B1_LOSS1_D6 = 1
B1_LOSS1_D7 = 1
B1_LOSS1_D8 = 1
B1_LOSS1_D9 = 2
B1_LOSS1_D10 = 1
B1_LOSS1_D11 = 1
B1_LOSS1_D12 = 2
B1_LOSS1_D13 = 2
B1_LOSS1_D14 = 2
B1_LOSS1_D15 = 2
B1_LOSS1_D16 = 1
B1_LOSS1_D17 = 2
B1_LOSS1_D18 = 2
B1_LOSS1_D19 = 1
B1_LOSS1_D20 = 1
B1_LOSS1_D21 = 2

# Block 1 Loss 2
B1_LOSS2_D1 = 1
B1_LOSS2_D2 = 2
B1_LOSS2_D3 = 1
B1_LOSS2_D4 = 1
B1_LOSS2_D5 = 2
B1_LOSS2_D6 = 2
B1_LOSS2_D7 = 1
B1_LOSS2_D8 = 1
B1_LOSS2_D9 = 2
B1_LOSS2_D10 = 2
B1_LOSS2_D11 = 1
B1_LOSS2_D12 = 1
B1_LOSS2_D13 = 1
B1_LOSS2_D14 = 2
B1_LOSS2_D15 = 1
B1_LOSS2_D16 = 1
B1_LOSS2_D17 = 2
B1_LOSS2_D18 = 2
B1_LOSS2_D19 = 1
B1_LOSS2_D20 = 2
B1_LOSS2_D21 = 1

# Block 1 Loss 3
B1_LOSS3_D1 = 1
B1_LOSS3_D2 = 1
B1_LOSS3_D3 = 1
B1_LOSS3_D4 = 2
B1_LOSS3_D5 = 1
B1_LOSS3_D6 = 2
B1_LOSS3_D7 = 1
B1_LOSS3_D8 = 1
B1_LOSS3_D9 = 2
B1_LOSS3_D10 = 2
B1_LOSS3_D11 = 1
B1_LOSS3_D12 = 2
B1_LOSS3_D13 = 1
B1_LOSS3_D14 = 2
B1_LOSS3_D15 = 2
B1_LOSS3_D16 = 2
B1_LOSS3_D17 = 2
B1_LOSS3_D18 = 1
B1_LOSS3_D19 = 2
B1_LOSS3_D20 = 2
B1_LOSS3_D21 = 1

# Block 1 Loss 4
B1_LOSS4_D1 = 1
B1_LOSS4_D2 = 1
B1_LOSS4_D3 = 2
B1_LOSS4_D4 = 1
B1_LOSS4_D5 = 2
B1_LOSS4_D6 = 2
B1_LOSS4_D7 = 1
B1_LOSS4_D8 = 1
B1_LOSS4_D9 = 1
B1_LOSS4_D10 = 1
B1_LOSS4_D11 = 2
B1_LOSS4_D12 = 1
B1_LOSS4_D13 = 2
B1_LOSS4_D14 = 2
B1_LOSS4_D15 = 2
B1_LOSS4_D16 = 1
B1_LOSS4_D17 = 2
B1_LOSS4_D18 = 1
B1_LOSS4_D19 = 1
B1_LOSS4_D20 = 2
B1_LOSS4_D21 = 2

# Block 1 Loss 5
B1_LOSS5_D1 = 1
B1_LOSS5_D2 = 2
B1_LOSS5_D3 = 1
B1_LOSS5_D4 = 1
B1_LOSS5_D5 = 1
B1_LOSS5_D6 = 2
B1_LOSS5_D7 = 1
B1_LOSS5_D8 = 2
B1_LOSS5_D9 = 1
B1_LOSS5_D10 = 1
B1_LOSS5_D11 = 2
B1_LOSS5_D12 = 2
B1_LOSS5_D13 = 2
B1_LOSS5_D14 = 2
B1_LOSS5_D15 = 2
B1_LOSS5_D16 = 1
B1_LOSS5_D17 = 2
B1_LOSS5_D18 = 2
B1_LOSS5_D19 = 2
B1_LOSS5_D20 = 1
B1_LOSS5_D21 = 2

# Block 1 Loss 6
B1_LOSS6_D1 = 1
B1_LOSS6_D2 = 1
B1_LOSS6_D3 = 2
B1_LOSS6_D4 = 2
B1_LOSS6_D5 = 1
B1_LOSS6_D6 = 2
B1_LOSS6_D7 = 1
B1_LOSS6_D8 = 2
B1_LOSS6_D9 = 2
B1_LOSS6_D10 = 2
B1_LOSS6_D11 = 1
B1_LOSS6_D12 = 1
B1_LOSS6_D13 = 1
B1_LOSS6_D14 = 1
B1_LOSS6_D15 = 1
B1_LOSS6_D16 = 2
B1_LOSS6_D17 = 1
B1_LOSS6_D18 = 1
B1_LOSS6_D19 = 2
B1_LOSS6_D20 = 2
B1_LOSS6_D21 = 1

# Block 1 Loss 7
B1_LOSS7_D1 = 1
B1_LOSS7_D2 = 1
B1_LOSS7_D3 = 2
B1_LOSS7_D4 = 1
B1_LOSS7_D5 = 1
B1_LOSS7_D6 = 1
B1_LOSS7_D7 = 2
B1_LOSS7_D8 = 1
B1_LOSS7_D9 = 2
B1_LOSS7_D10 = 2
B1_LOSS7_D11 = 1
B1_LOSS7_D12 = 1
B1_LOSS7_D13 = 2
B1_LOSS7_D14 = 2
B1_LOSS7_D15 = 1
B1_LOSS7_D16 = 1
B1_LOSS7_D17 = 2
B1_LOSS7_D18 = 2
B1_LOSS7_D19 = 2
B1_LOSS7_D20 = 2
B1_LOSS7_D21 = 1

# Block 1 Loss 8
B1_LOSS8_D1 = 2
B1_LOSS8_D2 = 1
B1_LOSS8_D3 = 2
B1_LOSS8_D4 = 2
B1_LOSS8_D5 = 2
B1_LOSS8_D6 = 1
B1_LOSS8_D7 = 2
B1_LOSS8_D8 = 2
B1_LOSS8_D9 = 2
B1_LOSS8_D10 = 2
B1_LOSS8_D11 = 1
B1_LOSS8_D12 = 2
B1_LOSS8_D13 = 2
B1_LOSS8_D14 = 2
B1_LOSS8_D15 = 1
B1_LOSS8_D16 = 1
B1_LOSS8_D17 = 1
B1_LOSS8_D18 = 2
B1_LOSS8_D19 = 2
B1_LOSS8_D20 = 1
B1_LOSS8_D21 = 1

# Block 1 Loss 9
B1_LOSS9_D1 = 2
B1_LOSS9_D2 = 1
B1_LOSS9_D3 = 1
B1_LOSS9_D4 = 2
B1_LOSS9_D5 = 2
B1_LOSS9_D6 = 2
B1_LOSS9_D7 = 1
B1_LOSS9_D8 = 2
B1_LOSS9_D9 = 2
B1_LOSS9_D10 = 1
B1_LOSS9_D11 = 1
B1_LOSS9_D12 = 2
B1_LOSS9_D13 = 1
B1_LOSS9_D14 = 1
B1_LOSS9_D15 = 1
B1_LOSS9_D16 = 2
B1_LOSS9_D17 = 1
B1_LOSS9_D18 = 1
B1_LOSS9_D19 = 2
B1_LOSS9_D20 = 1
B1_LOSS9_D21 = 2

# Block 1 Loss 10
B1_LOSS10_D1 = 2
B1_LOSS10_D2 = 2
B1_LOSS10_D3 = 2
B1_LOSS10_D4 = 1
B1_LOSS10_D5 = 2
B1_LOSS10_D6 = 1
B1_LOSS10_D7 = 2
B1_LOSS10_D8 = 2
B1_LOSS10_D9 = 1
B1_LOSS10_D10 = 2
B1_LOSS10_D11 = 1
B1_LOSS10_D12 = 2
B1_LOSS10_D13 = 1
B1_LOSS10_D14 = 1
B1_LOSS10_D15 = 1
B1_LOSS10_D16 = 2
B1_LOSS10_D17 = 1
B1_LOSS10_D18 = 1
B1_LOSS10_D19 = 2
B1_LOSS10_D20 = 1
B1_LOSS10_D21 = 2

# Block 1 Loss 11
B1_LOSS11_D1 = 1
B1_LOSS11_D2 = 2
B1_LOSS11_D3 = 2
B1_LOSS11_D4 = 1
B1_LOSS11_D5 = 2
B1_LOSS11_D6 = 1
B1_LOSS11_D7 = 2
B1_LOSS11_D8 = 1
B1_LOSS11_D9 = 2
B1_LOSS11_D10 = 1
B1_LOSS11_D11 = 1
B1_LOSS11_D12 = 1
B1_LOSS11_D13 = 2
B1_LOSS11_D14 = 2
B1_LOSS11_D15 = 1
B1_LOSS11_D16 = 2
B1_LOSS11_D17 = 2
B1_LOSS11_D18 = 1
B1_LOSS11_D19 = 1
B1_LOSS11_D20 = 1
B1_LOSS11_D21 = 1

# Block 1 Loss 12
B1_LOSS12_D1 = 2
B1_LOSS12_D2 = 2
B1_LOSS12_D3 = 1
B1_LOSS12_D4 = 1
B1_LOSS12_D5 = 1
B1_LOSS12_D6 = 2
B1_LOSS12_D7 = 1
B1_LOSS12_D8 = 2
B1_LOSS12_D9 = 1
B1_LOSS12_D10 = 1
B1_LOSS12_D11 = 1
B1_LOSS12_D12 = 2
B1_LOSS12_D13 = 1
B1_LOSS12_D14 = 2
B1_LOSS12_D15 = 1
B1_LOSS12_D16 = 1
B1_LOSS12_D17 = 2
B1_LOSS12_D18 = 1
B1_LOSS12_D19 = 1
B1_LOSS12_D20 = 2
B1_LOSS12_D21 = 2

# Block 1 Loss 13
B1_LOSS13_D1 = 2
B1_LOSS13_D2 = 1
B1_LOSS13_D3 = 1
B1_LOSS13_D4 = 1
B1_LOSS13_D5 = 1
B1_LOSS13_D6 = 1
B1_LOSS13_D7 = 2
B1_LOSS13_D8 = 2
B1_LOSS13_D9 = 1
B1_LOSS13_D10 = 1
B1_LOSS13_D11 = 2
B1_LOSS13_D12 = 2
B1_LOSS13_D13 = 2
B1_LOSS13_D14 = 1
B1_LOSS13_D15 = 2
B1_LOSS13_D16 = 2
B1_LOSS13_D17 = 1
B1_LOSS13_D18 = 2
B1_LOSS13_D19 = 2
B1_LOSS13_D20 = 2
B1_LOSS13_D21 = 2

# Block 1 Loss 14
B1_LOSS14_D1 = 2
B1_LOSS14_D2 = 2
B1_LOSS14_D3 = 1
B1_LOSS14_D4 = 2
B1_LOSS14_D5 = 1
B1_LOSS14_D6 = 2
B1_LOSS14_D7 = 1
B1_LOSS14_D8 = 2
B1_LOSS14_D9 = 1
B1_LOSS14_D10 = 2
B1_LOSS14_D11 = 2
B1_LOSS14_D12 = 1
B1_LOSS14_D13 = 1
B1_LOSS14_D14 = 2
B1_LOSS14_D15 = 2
B1_LOSS14_D16 = 1
B1_LOSS14_D17 = 1
B1_LOSS14_D18 = 2
B1_LOSS14_D19 = 1
B1_LOSS14_D20 = 1
B1_LOSS14_D21 = 2

# Block 1 Loss 15
B1_LOSS15_D1 = 2
B1_LOSS15_D2 = 1
B1_LOSS15_D3 = 1
B1_LOSS15_D4 = 2
B1_LOSS15_D5 = 1
B1_LOSS15_D6 = 2
B1_LOSS15_D7 = 1
B1_LOSS15_D8 = 2
B1_LOSS15_D9 = 2
B1_LOSS15_D10 = 2
B1_LOSS15_D11 = 1
B1_LOSS15_D12 = 1
B1_LOSS15_D13 = 1
B1_LOSS15_D14 = 2
B1_LOSS15_D15 = 2
B1_LOSS15_D16 = 2
B1_LOSS15_D17 = 2
B1_LOSS15_D18 = 1
B1_LOSS15_D19 = 2
B1_LOSS15_D20 = 2
B1_LOSS15_D21 = 2

# Block 1 Loss 16
B1_LOSS16_D1 = 1
B1_LOSS16_D2 = 2
B1_LOSS16_D3 = 2
B1_LOSS16_D4 = 1
B1_LOSS16_D5 = 1
B1_LOSS16_D6 = 2
B1_LOSS16_D7 = 2
B1_LOSS16_D8 = 2
B1_LOSS16_D9 = 1
B1_LOSS16_D10 = 1
B1_LOSS16_D11 = 1
B1_LOSS16_D12 = 2
B1_LOSS16_D13 = 2
B1_LOSS16_D14 = 1
B1_LOSS16_D15 = 1
B1_LOSS16_D16 = 2
B1_LOSS16_D17 = 2
B1_LOSS16_D18 = 1
B1_LOSS16_D19 = 2
B1_LOSS16_D20 = 1
B1_LOSS16_D21 = 1

# Block 1 Loss 17
B1_LOSS17_D1 = 1
B1_LOSS17_D2 = 2
B1_LOSS17_D3 = 2
B1_LOSS17_D4 = 1
B1_LOSS17_D5 = 2
B1_LOSS17_D6 = 2
B1_LOSS17_D7 = 1
B1_LOSS17_D8 = 1
B1_LOSS17_D9 = 2
B1_LOSS17_D10 = 1
B1_LOSS17_D11 = 1
B1_LOSS17_D12 = 2
B1_LOSS17_D13 = 1
B1_LOSS17_D14 = 2
B1_LOSS17_D15 = 1
B1_LOSS17_D16 = 1
B1_LOSS17_D17 = 1
B1_LOSS17_D18 = 1
B1_LOSS17_D19 = 2
B1_LOSS17_D20 = 2
B1_LOSS17_D21 = 1

# Block 1 Loss 18
B1_LOSS18_D1 = 1
B1_LOSS18_D2 = 2
B1_LOSS18_D3 = 2
B1_LOSS18_D4 = 1
B1_LOSS18_D5 = 2
B1_LOSS18_D6 = 2
B1_LOSS18_D7 = 2
B1_LOSS18_D8 = 1
B1_LOSS18_D9 = 1
B1_LOSS18_D10 = 1
B1_LOSS18_D11 = 2
B1_LOSS18_D12 = 2
B1_LOSS18_D13 = 2
B1_LOSS18_D14 = 1
B1_LOSS18_D15 = 2
B1_LOSS18_D16 = 2
B1_LOSS18_D17 = 2
B1_LOSS18_D18 = 1
B1_LOSS18_D19 = 1
B1_LOSS18_D20 = 2
B1_LOSS18_D21 = 2

# Block 1 Loss 19
B1_LOSS19_D1 = 2
B1_LOSS19_D2 = 2
B1_LOSS19_D3 = 2
B1_LOSS19_D4 = 1
B1_LOSS19_D5 = 1
B1_LOSS19_D6 = 1
B1_LOSS19_D7 = 1
B1_LOSS19_D8 = 1
B1_LOSS19_D9 = 1
B1_LOSS19_D10 = 2
B1_LOSS19_D11 = 1
B1_LOSS19_D12 = 1
B1_LOSS19_D13 = 2
B1_LOSS19_D14 = 1
B1_LOSS19_D15 = 2
B1_LOSS19_D16 = 2
B1_LOSS19_D17 = 1
B1_LOSS19_D18 = 2
B1_LOSS19_D19 = 2
B1_LOSS19_D20 = 1
B1_LOSS19_D21 = 2

# Block 1 Loss 20
B1_LOSS20_D1 = 1
B1_LOSS20_D2 = 2
B1_LOSS20_D3 = 1
B1_LOSS20_D4 = 2
B1_LOSS20_D5 = 2
B1_LOSS20_D6 = 2
B1_LOSS20_D7 = 1
B1_LOSS20_D8 = 1
B1_LOSS20_D9 = 1
B1_LOSS20_D10 = 1
B1_LOSS20_D11 = 2
B1_LOSS20_D12 = 1
B1_LOSS20_D13 = 1
B1_LOSS20_D14 = 1
B1_LOSS20_D15 = 2
B1_LOSS20_D16 = 2
B1_LOSS20_D17 = 2
B1_LOSS20_D18 = 1
B1_LOSS20_D19 = 2
B1_LOSS20_D20 = 1
B1_LOSS20_D21 = 2

# Block 2 Gain 1
B2_GAIN1_D1 = 1
B2_GAIN1_D2 = 1
B2_GAIN1_D3 = 1
B2_GAIN1_D4 = 1
B2_GAIN1_D5 = 2
B2_GAIN1_D6 = 2
B2_GAIN1_D7 = 2
B2_GAIN1_D8 = 1
B2_GAIN1_D9 = 1
B2_GAIN1_D10 = 2
B2_GAIN1_D11 = 2
B2_GAIN1_D12 = 2
B2_GAIN1_D13 = 1
B2_GAIN1_D14 = 2
B2_GAIN1_D15 = 2
B2_GAIN1_D16 = 2
B2_GAIN1_D17 = 1
B2_GAIN1_D18 = 1
B2_GAIN1_D19 = 1
B2_GAIN1_D20 = 2
B2_GAIN1_D21 = 1

# Block 2 Gain 2
B2_GAIN2_D1 = 1
B2_GAIN2_D2 = 1
B2_GAIN2_D3 = 1
B2_GAIN2_D4 = 2
B2_GAIN2_D5 = 2
B2_GAIN2_D6 = 2
B2_GAIN2_D7 = 2
B2_GAIN2_D8 = 2
B2_GAIN2_D9 = 1
B2_GAIN2_D10 = 2
B2_GAIN2_D11 = 2
B2_GAIN2_D12 = 2
B2_GAIN2_D13 = 2
B2_GAIN2_D14 = 2
B2_GAIN2_D15 = 1
B2_GAIN2_D16 = 1
B2_GAIN2_D17 = 2
B2_GAIN2_D18 = 1
B2_GAIN2_D19 = 1
B2_GAIN2_D20 = 2
B2_GAIN2_D21 = 1

# Block 2 Gain 3
B2_GAIN3_D1 = 2
B2_GAIN3_D2 = 2
B2_GAIN3_D3 = 2
B2_GAIN3_D4 = 1
B2_GAIN3_D5 = 1
B2_GAIN3_D6 = 2
B2_GAIN3_D7 = 1
B2_GAIN3_D8 = 1
B2_GAIN3_D9 = 1
B2_GAIN3_D10 = 2
B2_GAIN3_D11 = 2
B2_GAIN3_D12 = 1
B2_GAIN3_D13 = 2
B2_GAIN3_D14 = 2
B2_GAIN3_D15 = 1
B2_GAIN3_D16 = 2
B2_GAIN3_D17 = 1
B2_GAIN3_D18 = 1
B2_GAIN3_D19 = 1
B2_GAIN3_D20 = 2
B2_GAIN3_D21 = 1

# Block 2 Gain 4
B2_GAIN4_D1 = 2
B2_GAIN4_D2 = 2
B2_GAIN4_D3 = 2
B2_GAIN4_D4 = 2
B2_GAIN4_D5 = 1
B2_GAIN4_D6 = 2
B2_GAIN4_D7 = 1
B2_GAIN4_D8 = 2
B2_GAIN4_D9 = 1
B2_GAIN4_D10 = 1
B2_GAIN4_D11 = 2
B2_GAIN4_D12 = 1
B2_GAIN4_D13 = 2
B2_GAIN4_D14 = 2
B2_GAIN4_D15 = 2
B2_GAIN4_D16 = 2
B2_GAIN4_D17 = 1
B2_GAIN4_D18 = 1
B2_GAIN4_D19 = 1
B2_GAIN4_D20 = 2
B2_GAIN4_D21 = 2

# Block 2 Gain 5
B2_GAIN5_D1 = 2
B2_GAIN5_D2 = 2
B2_GAIN5_D3 = 1
B2_GAIN5_D4 = 2
B2_GAIN5_D5 = 1
B2_GAIN5_D6 = 2
B2_GAIN5_D7 = 1
B2_GAIN5_D8 = 2
B2_GAIN5_D9 = 2
B2_GAIN5_D10 = 2
B2_GAIN5_D11 = 2
B2_GAIN5_D12 = 1
B2_GAIN5_D13 = 2
B2_GAIN5_D14 = 1
B2_GAIN5_D15 = 1
B2_GAIN5_D16 = 1
B2_GAIN5_D17 = 1
B2_GAIN5_D18 = 2
B2_GAIN5_D19 = 1
B2_GAIN5_D20 = 2
B2_GAIN5_D21 = 1

# Block 2 Gain 6
B2_GAIN6_D1 = 1
B2_GAIN6_D2 = 1
B2_GAIN6_D3 = 2
B2_GAIN6_D4 = 1
B2_GAIN6_D5 = 1
B2_GAIN6_D6 = 2
B2_GAIN6_D7 = 2
B2_GAIN6_D8 = 1
B2_GAIN6_D9 = 1
B2_GAIN6_D10 = 2
B2_GAIN6_D11 = 1
B2_GAIN6_D12 = 2
B2_GAIN6_D13 = 1
B2_GAIN6_D14 = 2
B2_GAIN6_D15 = 2
B2_GAIN6_D16 = 2
B2_GAIN6_D17 = 2
B2_GAIN6_D18 = 1
B2_GAIN6_D19 = 1
B2_GAIN6_D20 = 2
B2_GAIN6_D21 = 1

# Block 2 Gain 7
B2_GAIN7_D1 = 1
B2_GAIN7_D2 = 1
B2_GAIN7_D3 = 1
B2_GAIN7_D4 = 1
B2_GAIN7_D5 = 1
B2_GAIN7_D6 = 2
B2_GAIN7_D7 = 2
B2_GAIN7_D8 = 2
B2_GAIN7_D9 = 2
B2_GAIN7_D10 = 1
B2_GAIN7_D11 = 1
B2_GAIN7_D12 = 1
B2_GAIN7_D13 = 2
B2_GAIN7_D14 = 1
B2_GAIN7_D15 = 1
B2_GAIN7_D16 = 1
B2_GAIN7_D17 = 2
B2_GAIN7_D18 = 2
B2_GAIN7_D19 = 1
B2_GAIN7_D20 = 1
B2_GAIN7_D21 = 2

# Block 2 Gain 8
B2_GAIN8_D1 = 2
B2_GAIN8_D2 = 1
B2_GAIN8_D3 = 1
B2_GAIN8_D4 = 1
B2_GAIN8_D5 = 1
B2_GAIN8_D6 = 1
B2_GAIN8_D7 = 1
B2_GAIN8_D8 = 2
B2_GAIN8_D9 = 1
B2_GAIN8_D10 = 1
B2_GAIN8_D11 = 2
B2_GAIN8_D12 = 1
B2_GAIN8_D13 = 2
B2_GAIN8_D14 = 2
B2_GAIN8_D15 = 2
B2_GAIN8_D16 = 2
B2_GAIN8_D17 = 1
B2_GAIN8_D18 = 2
B2_GAIN8_D19 = 1
B2_GAIN8_D20 = 2
B2_GAIN8_D21 = 1

# Block 2 Gain 9
B2_GAIN9_D1 = 2
B2_GAIN9_D2 = 1
B2_GAIN9_D3 = 2
B2_GAIN9_D4 = 1
B2_GAIN9_D5 = 1
B2_GAIN9_D6 = 1
B2_GAIN9_D7 = 1
B2_GAIN9_D8 = 2
B2_GAIN9_D9 = 1
B2_GAIN9_D10 = 2
B2_GAIN9_D11 = 2
B2_GAIN9_D12 = 1
B2_GAIN9_D13 = 1
B2_GAIN9_D14 = 1
B2_GAIN9_D15 = 1
B2_GAIN9_D16 = 1
B2_GAIN9_D17 = 1
B2_GAIN9_D18 = 2
B2_GAIN9_D19 = 2
B2_GAIN9_D20 = 1
B2_GAIN9_D21 = 2

# Block 2 Gain 10
B2_GAIN10_D1 = 1
B2_GAIN10_D2 = 2
B2_GAIN10_D3 = 1
B2_GAIN10_D4 = 2
B2_GAIN10_D5 = 2
B2_GAIN10_D6 = 1
B2_GAIN10_D7 = 2
B2_GAIN10_D8 = 2
B2_GAIN10_D9 = 2
B2_GAIN10_D10 = 1
B2_GAIN10_D11 = 1
B2_GAIN10_D12 = 2
B2_GAIN10_D13 = 2
B2_GAIN10_D14 = 1
B2_GAIN10_D15 = 1
B2_GAIN10_D16 = 1
B2_GAIN10_D17 = 2
B2_GAIN10_D18 = 2
B2_GAIN10_D19 = 1
B2_GAIN10_D20 = 2
B2_GAIN10_D21 = 2

# Block 2 Gain 11
B2_GAIN11_D1 = 2
B2_GAIN11_D2 = 2
B2_GAIN11_D3 = 1
B2_GAIN11_D4 = 2
B2_GAIN11_D5 = 1
B2_GAIN11_D6 = 1
B2_GAIN11_D7 = 2
B2_GAIN11_D8 = 2
B2_GAIN11_D9 = 2
B2_GAIN11_D10 = 2
B2_GAIN11_D11 = 2
B2_GAIN11_D12 = 2
B2_GAIN11_D13 = 1
B2_GAIN11_D14 = 2
B2_GAIN11_D15 = 1
B2_GAIN11_D16 = 2
B2_GAIN11_D17 = 1
B2_GAIN11_D18 = 1
B2_GAIN11_D19 = 2
B2_GAIN11_D20 = 2
B2_GAIN11_D21 = 1

# Block 2 Gain 12
B2_GAIN12_D1 = 1
B2_GAIN12_D2 = 2
B2_GAIN12_D3 = 2
B2_GAIN12_D4 = 2
B2_GAIN12_D5 = 2
B2_GAIN12_D6 = 1
B2_GAIN12_D7 = 1
B2_GAIN12_D8 = 2
B2_GAIN12_D9 = 1
B2_GAIN12_D10 = 2
B2_GAIN12_D11 = 1
B2_GAIN12_D12 = 1
B2_GAIN12_D13 = 2
B2_GAIN12_D14 = 2
B2_GAIN12_D15 = 1
B2_GAIN12_D16 = 1
B2_GAIN12_D17 = 2
B2_GAIN12_D18 = 2
B2_GAIN12_D19 = 2
B2_GAIN12_D20 = 2
B2_GAIN12_D21 = 2

# Block 2 Gain 13
B2_GAIN13_D1 = 2
B2_GAIN13_D2 = 1
B2_GAIN13_D3 = 1
B2_GAIN13_D4 = 2
B2_GAIN13_D5 = 1
B2_GAIN13_D6 = 2
B2_GAIN13_D7 = 1
B2_GAIN13_D8 = 1
B2_GAIN13_D9 = 1
B2_GAIN13_D10 = 1
B2_GAIN13_D11 = 2
B2_GAIN13_D12 = 1
B2_GAIN13_D13 = 1
B2_GAIN13_D14 = 1
B2_GAIN13_D15 = 2
B2_GAIN13_D16 = 1
B2_GAIN13_D17 = 2
B2_GAIN13_D18 = 2
B2_GAIN13_D19 = 2
B2_GAIN13_D20 = 2
B2_GAIN13_D21 = 1

# Block 2 Gain 14
B2_GAIN14_D1 = 1
B2_GAIN14_D2 = 1
B2_GAIN14_D3 = 2
B2_GAIN14_D4 = 2
B2_GAIN14_D5 = 1
B2_GAIN14_D6 = 2
B2_GAIN14_D7 = 2
B2_GAIN14_D8 = 1
B2_GAIN14_D9 = 2
B2_GAIN14_D10 = 1
B2_GAIN14_D11 = 2
B2_GAIN14_D12 = 1
B2_GAIN14_D13 = 1
B2_GAIN14_D14 = 1
B2_GAIN14_D15 = 1
B2_GAIN14_D16 = 2
B2_GAIN14_D17 = 1
B2_GAIN14_D18 = 1
B2_GAIN14_D19 = 1
B2_GAIN14_D20 = 1
B2_GAIN14_D21 = 1

# Block 2 Gain 15
B2_GAIN15_D1 = 1
B2_GAIN15_D2 = 1
B2_GAIN15_D3 = 1
B2_GAIN15_D4 = 2
B2_GAIN15_D5 = 1
B2_GAIN15_D6 = 2
B2_GAIN15_D7 = 1
B2_GAIN15_D8 = 1
B2_GAIN15_D9 = 2
B2_GAIN15_D10 = 2
B2_GAIN15_D11 = 2
B2_GAIN15_D12 = 2
B2_GAIN15_D13 = 2
B2_GAIN15_D14 = 1
B2_GAIN15_D15 = 1
B2_GAIN15_D16 = 2
B2_GAIN15_D17 = 1
B2_GAIN15_D18 = 1
B2_GAIN15_D19 = 1
B2_GAIN15_D20 = 2
B2_GAIN15_D21 = 2

# Block 2 Gain 16
B2_GAIN16_D1 = 2
B2_GAIN16_D2 = 2
B2_GAIN16_D3 = 1
B2_GAIN16_D4 = 2
B2_GAIN16_D5 = 2
B2_GAIN16_D6 = 2
B2_GAIN16_D7 = 1
B2_GAIN16_D8 = 1
B2_GAIN16_D9 = 1
B2_GAIN16_D10 = 1
B2_GAIN16_D11 = 1
B2_GAIN16_D12 = 2
B2_GAIN16_D13 = 2
B2_GAIN16_D14 = 1
B2_GAIN16_D15 = 1
B2_GAIN16_D16 = 2
B2_GAIN16_D17 = 1
B2_GAIN16_D18 = 2
B2_GAIN16_D19 = 2
B2_GAIN16_D20 = 2
B2_GAIN16_D21 = 2

# Block 2 Gain 17
B2_GAIN17_D1 = 1
B2_GAIN17_D2 = 1
B2_GAIN17_D3 = 2
B2_GAIN17_D4 = 2
B2_GAIN17_D5 = 1
B2_GAIN17_D6 = 2
B2_GAIN17_D7 = 1
B2_GAIN17_D8 = 1
B2_GAIN17_D9 = 2
B2_GAIN17_D10 = 1
B2_GAIN17_D11 = 1
B2_GAIN17_D12 = 2
B2_GAIN17_D13 = 1
B2_GAIN17_D14 = 1
B2_GAIN17_D15 = 1
B2_GAIN17_D16 = 2
B2_GAIN17_D17 = 1
B2_GAIN17_D18 = 2
B2_GAIN17_D19 = 2
B2_GAIN17_D20 = 1
B2_GAIN17_D21 = 2

# Block 2 Gain 18
B2_GAIN18_D1 = 1
B2_GAIN18_D2 = 1
B2_GAIN18_D3 = 1
B2_GAIN18_D4 = 1
B2_GAIN18_D5 = 1
B2_GAIN18_D6 = 2
B2_GAIN18_D7 = 1
B2_GAIN18_D8 = 1
B2_GAIN18_D9 = 2
B2_GAIN18_D10 = 1
B2_GAIN18_D11 = 1
B2_GAIN18_D12 = 2
B2_GAIN18_D13 = 2
B2_GAIN18_D14 = 2
B2_GAIN18_D15 = 1
B2_GAIN18_D16 = 2
B2_GAIN18_D17 = 2
B2_GAIN18_D18 = 2
B2_GAIN18_D19 = 2
B2_GAIN18_D20 = 1
B2_GAIN18_D21 = 2

# Block 2 Gain 19
B2_GAIN19_D1 = 2
B2_GAIN19_D2 = 1
B2_GAIN19_D3 = 1
B2_GAIN19_D4 = 1
B2_GAIN19_D5 = 2
B2_GAIN19_D6 = 2
B2_GAIN19_D7 = 2
B2_GAIN19_D8 = 2
B2_GAIN19_D9 = 1
B2_GAIN19_D10 = 1
B2_GAIN19_D11 = 2
B2_GAIN19_D12 = 2
B2_GAIN19_D13 = 2
B2_GAIN19_D14 = 2
B2_GAIN19_D15 = 1
B2_GAIN19_D16 = 1
B2_GAIN19_D17 = 2
B2_GAIN19_D18 = 1
B2_GAIN19_D19 = 2
B2_GAIN19_D20 = 1
B2_GAIN19_D21 = 2

# Block 2 Gain 20
B2_GAIN20_D1 = 2
B2_GAIN20_D2 = 2
B2_GAIN20_D3 = 1
B2_GAIN20_D4 = 2
B2_GAIN20_D5 = 2
B2_GAIN20_D6 = 2
B2_GAIN20_D7 = 2
B2_GAIN20_D8 = 2
B2_GAIN20_D9 = 1
B2_GAIN20_D10 = 1
B2_GAIN20_D11 = 2
B2_GAIN20_D12 = 1
B2_GAIN20_D13 = 1
B2_GAIN20_D14 = 2
B2_GAIN20_D15 = 2
B2_GAIN20_D16 = 1
B2_GAIN20_D17 = 2
B2_GAIN20_D18 = 2
B2_GAIN20_D19 = 2
B2_GAIN20_D20 = 1
B2_GAIN20_D21 = 1

# Block 2 Loss 1
B2_LOSS1_D1 = 1
B2_LOSS1_D2 = 1
B2_LOSS1_D3 = 2
B2_LOSS1_D4 = 2
B2_LOSS1_D5 = 2
B2_LOSS1_D6 = 1
B2_LOSS1_D7 = 1
B2_LOSS1_D8 = 2
B2_LOSS1_D9 = 1
B2_LOSS1_D10 = 2
B2_LOSS1_D11 = 2
B2_LOSS1_D12 = 2
B2_LOSS1_D13 = 2
B2_LOSS1_D14 = 2
B2_LOSS1_D15 = 1
B2_LOSS1_D16 = 2
B2_LOSS1_D17 = 2
B2_LOSS1_D18 = 2
B2_LOSS1_D19 = 1
B2_LOSS1_D20 = 2
B2_LOSS1_D21 = 1

# Block 2 Loss 2
B2_LOSS2_D1 = 1
B2_LOSS2_D2 = 1
B2_LOSS2_D3 = 1
B2_LOSS2_D4 = 1
B2_LOSS2_D5 = 1
B2_LOSS2_D6 = 2
B2_LOSS2_D7 = 1
B2_LOSS2_D8 = 1
B2_LOSS2_D9 = 1
B2_LOSS2_D10 = 1
B2_LOSS2_D11 = 1
B2_LOSS2_D12 = 1
B2_LOSS2_D13 = 1
B2_LOSS2_D14 = 1
B2_LOSS2_D15 = 2
B2_LOSS2_D16 = 2
B2_LOSS2_D17 = 1
B2_LOSS2_D18 = 2
B2_LOSS2_D19 = 1
B2_LOSS2_D20 = 1
B2_LOSS2_D21 = 2

# Block 2 Loss 3
B2_LOSS3_D1 = 1
B2_LOSS3_D2 = 1
B2_LOSS3_D3 = 1
B2_LOSS3_D4 = 1
B2_LOSS3_D5 = 1
B2_LOSS3_D6 = 2
B2_LOSS3_D7 = 1
B2_LOSS3_D8 = 1
B2_LOSS3_D9 = 1
B2_LOSS3_D10 = 2
B2_LOSS3_D11 = 1
B2_LOSS3_D12 = 1
B2_LOSS3_D13 = 2
B2_LOSS3_D14 = 1
B2_LOSS3_D15 = 2
B2_LOSS3_D16 = 2
B2_LOSS3_D17 = 2
B2_LOSS3_D18 = 1
B2_LOSS3_D19 = 2
B2_LOSS3_D20 = 1
B2_LOSS3_D21 = 2

# Block 2 Loss 4
B2_LOSS4_D1 = 2
B2_LOSS4_D2 = 1
B2_LOSS4_D3 = 2
B2_LOSS4_D4 = 2
B2_LOSS4_D5 = 2
B2_LOSS4_D6 = 2
B2_LOSS4_D7 = 1
B2_LOSS4_D8 = 2
B2_LOSS4_D9 = 2
B2_LOSS4_D10 = 1
B2_LOSS4_D11 = 2
B2_LOSS4_D12 = 2
B2_LOSS4_D13 = 2
B2_LOSS4_D14 = 2
B2_LOSS4_D15 = 1
B2_LOSS4_D16 = 1
B2_LOSS4_D17 = 1
B2_LOSS4_D18 = 1
B2_LOSS4_D19 = 1
B2_LOSS4_D20 = 1
B2_LOSS4_D21 = 2

# Block 2 Loss 5
B2_LOSS5_D1 = 1
B2_LOSS5_D2 = 1
B2_LOSS5_D3 = 1
B2_LOSS5_D4 = 2
B2_LOSS5_D5 = 2
B2_LOSS5_D6 = 1
B2_LOSS5_D7 = 1
B2_LOSS5_D8 = 2
B2_LOSS5_D9 = 1
B2_LOSS5_D10 = 2
B2_LOSS5_D11 = 2
B2_LOSS5_D12 = 1
B2_LOSS5_D13 = 1
B2_LOSS5_D14 = 1
B2_LOSS5_D15 = 1
B2_LOSS5_D16 = 1
B2_LOSS5_D17 = 1
B2_LOSS5_D18 = 2
B2_LOSS5_D19 = 1
B2_LOSS5_D20 = 1
B2_LOSS5_D21 = 1

# Block 2 Loss 6
B2_LOSS6_D1 = 1
B2_LOSS6_D2 = 2
B2_LOSS6_D3 = 2
B2_LOSS6_D4 = 2
B2_LOSS6_D5 = 1
B2_LOSS6_D6 = 2
B2_LOSS6_D7 = 2
B2_LOSS6_D8 = 1
B2_LOSS6_D9 = 2
B2_LOSS6_D10 = 2
B2_LOSS6_D11 = 2
B2_LOSS6_D12 = 1
B2_LOSS6_D13 = 2
B2_LOSS6_D14 = 2
B2_LOSS6_D15 = 2
B2_LOSS6_D16 = 2
B2_LOSS6_D17 = 2
B2_LOSS6_D18 = 1
B2_LOSS6_D19 = 1
B2_LOSS6_D20 = 2
B2_LOSS6_D21 = 2

# Block 2 Loss 7
B2_LOSS7_D1 = 1
B2_LOSS7_D2 = 2
B2_LOSS7_D3 = 2
B2_LOSS7_D4 = 1
B2_LOSS7_D5 = 1
B2_LOSS7_D6 = 1
B2_LOSS7_D7 = 1
B2_LOSS7_D8 = 1
B2_LOSS7_D9 = 2
B2_LOSS7_D10 = 1
B2_LOSS7_D11 = 2
B2_LOSS7_D12 = 2
B2_LOSS7_D13 = 1
B2_LOSS7_D14 = 1
B2_LOSS7_D15 = 1
B2_LOSS7_D16 = 1
B2_LOSS7_D17 = 1
B2_LOSS7_D18 = 1
B2_LOSS7_D19 = 2
B2_LOSS7_D20 = 1
B2_LOSS7_D21 = 1

# Block 2 Loss 8
B2_LOSS8_D1 = 2
B2_LOSS8_D2 = 2
B2_LOSS8_D3 = 2
B2_LOSS8_D4 = 1
B2_LOSS8_D5 = 2
B2_LOSS8_D6 = 2
B2_LOSS8_D7 = 2
B2_LOSS8_D8 = 2
B2_LOSS8_D9 = 2
B2_LOSS8_D10 = 1
B2_LOSS8_D11 = 1
B2_LOSS8_D12 = 2
B2_LOSS8_D13 = 1
B2_LOSS8_D14 = 1
B2_LOSS8_D15 = 2
B2_LOSS8_D16 = 2
B2_LOSS8_D17 = 1
B2_LOSS8_D18 = 2
B2_LOSS8_D19 = 1
B2_LOSS8_D20 = 2
B2_LOSS8_D21 = 2

# Block 2 Loss 9
B2_LOSS9_D1 = 2
B2_LOSS9_D2 = 1
B2_LOSS9_D3 = 2
B2_LOSS9_D4 = 2
B2_LOSS9_D5 = 2
B2_LOSS9_D6 = 2
B2_LOSS9_D7 = 2
B2_LOSS9_D8 = 1
B2_LOSS9_D9 = 2
B2_LOSS9_D10 = 2
B2_LOSS9_D11 = 1
B2_LOSS9_D12 = 2
B2_LOSS9_D13 = 1
B2_LOSS9_D14 = 1
B2_LOSS9_D15 = 2
B2_LOSS9_D16 = 2
B2_LOSS9_D17 = 1
B2_LOSS9_D18 = 1
B2_LOSS9_D19 = 2
B2_LOSS9_D20 = 1
B2_LOSS9_D21 = 1

# Block 2 Loss 10
B2_LOSS10_D1 = 2
B2_LOSS10_D2 = 1
B2_LOSS10_D3 = 1
B2_LOSS10_D4 = 1
B2_LOSS10_D5 = 1
B2_LOSS10_D6 = 2
B2_LOSS10_D7 = 1
B2_LOSS10_D8 = 1
B2_LOSS10_D9 = 2
B2_LOSS10_D10 = 2
B2_LOSS10_D11 = 1
B2_LOSS10_D12 = 2
B2_LOSS10_D13 = 1
B2_LOSS10_D14 = 2
B2_LOSS10_D15 = 2
B2_LOSS10_D16 = 2
B2_LOSS10_D17 = 2
B2_LOSS10_D18 = 1
B2_LOSS10_D19 = 2
B2_LOSS10_D20 = 2
B2_LOSS10_D21 = 2

# Block 2 Loss 11
B2_LOSS11_D1 = 2
B2_LOSS11_D2 = 1
B2_LOSS11_D3 = 1
B2_LOSS11_D4 = 1
B2_LOSS11_D5 = 2
B2_LOSS11_D6 = 1
B2_LOSS11_D7 = 1
B2_LOSS11_D8 = 1
B2_LOSS11_D9 = 2
B2_LOSS11_D10 = 1
B2_LOSS11_D11 = 1
B2_LOSS11_D12 = 2
B2_LOSS11_D13 = 2
B2_LOSS11_D14 = 2
B2_LOSS11_D15 = 2
B2_LOSS11_D16 = 2
B2_LOSS11_D17 = 1
B2_LOSS11_D18 = 1
B2_LOSS11_D19 = 2
B2_LOSS11_D20 = 1
B2_LOSS11_D21 = 1

# Block 2 Loss 12
B2_LOSS12_D1 = 2
B2_LOSS12_D2 = 2
B2_LOSS12_D3 = 1
B2_LOSS12_D4 = 1
B2_LOSS12_D5 = 2
B2_LOSS12_D6 = 1
B2_LOSS12_D7 = 1
B2_LOSS12_D8 = 1
B2_LOSS12_D9 = 1
B2_LOSS12_D10 = 1
B2_LOSS12_D11 = 1
B2_LOSS12_D12 = 2
B2_LOSS12_D13 = 1
B2_LOSS12_D14 = 1
B2_LOSS12_D15 = 2
B2_LOSS12_D16 = 2
B2_LOSS12_D17 = 2
B2_LOSS12_D18 = 2
B2_LOSS12_D19 = 1
B2_LOSS12_D20 = 2
B2_LOSS12_D21 = 1

# Block 2 Loss 13
B2_LOSS13_D1 = 2
B2_LOSS13_D2 = 2
B2_LOSS13_D3 = 1
B2_LOSS13_D4 = 2
B2_LOSS13_D5 = 2
B2_LOSS13_D6 = 1
B2_LOSS13_D7 = 2
B2_LOSS13_D8 = 1
B2_LOSS13_D9 = 1
B2_LOSS13_D10 = 1
B2_LOSS13_D11 = 1
B2_LOSS13_D12 = 1
B2_LOSS13_D13 = 2
B2_LOSS13_D14 = 2
B2_LOSS13_D15 = 2
B2_LOSS13_D16 = 2
B2_LOSS13_D17 = 2
B2_LOSS13_D18 = 1
B2_LOSS13_D19 = 1
B2_LOSS13_D20 = 2
B2_LOSS13_D21 = 1

# Block 2 Loss 14
B2_LOSS14_D1 = 1
B2_LOSS14_D2 = 2
B2_LOSS14_D3 = 1
B2_LOSS14_D4 = 1
B2_LOSS14_D5 = 1
B2_LOSS14_D6 = 1
B2_LOSS14_D7 = 2
B2_LOSS14_D8 = 2
B2_LOSS14_D9 = 2
B2_LOSS14_D10 = 1
B2_LOSS14_D11 = 1
B2_LOSS14_D12 = 2
B2_LOSS14_D13 = 1
B2_LOSS14_D14 = 2
B2_LOSS14_D15 = 2
B2_LOSS14_D16 = 1
B2_LOSS14_D17 = 2
B2_LOSS14_D18 = 2
B2_LOSS14_D19 = 1
B2_LOSS14_D20 = 2
B2_LOSS14_D21 = 1

# Block 2 Loss 15
B2_LOSS15_D1 = 2
B2_LOSS15_D2 = 1
B2_LOSS15_D3 = 2
B2_LOSS15_D4 = 2
B2_LOSS15_D5 = 1
B2_LOSS15_D6 = 2
B2_LOSS15_D7 = 2
B2_LOSS15_D8 = 1
B2_LOSS15_D9 = 2
B2_LOSS15_D10 = 1
B2_LOSS15_D11 = 1
B2_LOSS15_D12 = 1
B2_LOSS15_D13 = 2
B2_LOSS15_D14 = 1
B2_LOSS15_D15 = 2
B2_LOSS15_D16 = 1
B2_LOSS15_D17 = 1
B2_LOSS15_D18 = 2
B2_LOSS15_D19 = 1
B2_LOSS15_D20 = 2
B2_LOSS15_D21 = 1

# Block 2 Loss 16
B2_LOSS16_D1 = 1
B2_LOSS16_D2 = 1
B2_LOSS16_D3 = 2
B2_LOSS16_D4 = 1
B2_LOSS16_D5 = 1
B2_LOSS16_D6 = 1
B2_LOSS16_D7 = 1
B2_LOSS16_D8 = 2
B2_LOSS16_D9 = 2
B2_LOSS16_D10 = 1
B2_LOSS16_D11 = 2
B2_LOSS16_D12 = 2
B2_LOSS16_D13 = 2
B2_LOSS16_D14 = 2
B2_LOSS16_D15 = 2
B2_LOSS16_D16 = 2
B2_LOSS16_D17 = 1
B2_LOSS16_D18 = 1
B2_LOSS16_D19 = 1
B2_LOSS16_D20 = 2
B2_LOSS16_D21 = 2

# Block 2 Loss 17
B2_LOSS17_D1 = 2
B2_LOSS17_D2 = 1
B2_LOSS17_D3 = 1
B2_LOSS17_D4 = 1
B2_LOSS17_D5 = 1
B2_LOSS17_D6 = 2
B2_LOSS17_D7 = 2
B2_LOSS17_D8 = 2
B2_LOSS17_D9 = 1
B2_LOSS17_D10 = 2
B2_LOSS17_D11 = 1
B2_LOSS17_D12 = 1
B2_LOSS17_D13 = 2
B2_LOSS17_D14 = 1
B2_LOSS17_D15 = 2
B2_LOSS17_D16 = 2
B2_LOSS17_D17 = 2
B2_LOSS17_D18 = 2
B2_LOSS17_D19 = 1
B2_LOSS17_D20 = 1
B2_LOSS17_D21 = 1

# Block 2 Loss 18
B2_LOSS18_D1 = 1
B2_LOSS18_D2 = 1
B2_LOSS18_D3 = 2
B2_LOSS18_D4 = 2
B2_LOSS18_D5 = 1
B2_LOSS18_D6 = 1
B2_LOSS18_D7 = 2
B2_LOSS18_D8 = 1
B2_LOSS18_D9 = 2
B2_LOSS18_D10 = 2
B2_LOSS18_D11 = 2
B2_LOSS18_D12 = 2
B2_LOSS18_D13 = 2
B2_LOSS18_D14 = 1
B2_LOSS18_D15 = 1
B2_LOSS18_D16 = 1
B2_LOSS18_D17 = 1
B2_LOSS18_D18 = 2
B2_LOSS18_D19 = 1
B2_LOSS18_D20 = 1
B2_LOSS18_D21 = 2

# Block 2 Loss 19
B2_LOSS19_D1 = 1
B2_LOSS19_D2 = 1
B2_LOSS19_D3 = 1
B2_LOSS19_D4 = 2
B2_LOSS19_D5 = 1
B2_LOSS19_D6 = 2
B2_LOSS19_D7 = 1
B2_LOSS19_D8 = 2
B2_LOSS19_D9 = 1
B2_LOSS19_D10 = 2
B2_LOSS19_D11 = 2
B2_LOSS19_D12 = 1
B2_LOSS19_D13 = 2
B2_LOSS19_D14 = 2
B2_LOSS19_D15 = 1
B2_LOSS19_D16 = 1
B2_LOSS19_D17 = 1
B2_LOSS19_D18 = 1
B2_LOSS19_D19 = 1
B2_LOSS19_D20 = 2
B2_LOSS19_D21 = 1

# Block 2 Loss 20
B2_LOSS20_D1 = 1
B2_LOSS20_D2 = 1
B2_LOSS20_D3 = 1
B2_LOSS20_D4 = 1
B2_LOSS20_D5 = 2
B2_LOSS20_D6 = 2
B2_LOSS20_D7 = 2
B2_LOSS20_D8 = 1
B2_LOSS20_D9 = 2
B2_LOSS20_D10 = 2
B2_LOSS20_D11 = 2
B2_LOSS20_D12 = 1
B2_LOSS20_D13 = 1
B2_LOSS20_D14 = 1
B2_LOSS20_D15 = 2
B2_LOSS20_D16 = 2
B2_LOSS20_D17 = 1
B2_LOSS20_D18 = 1
B2_LOSS20_D19 = 2
B2_LOSS20_D20 = 2
B2_LOSS20_D21 = 1


def getPayoff():
        # Randomizer
        rd_typ = rd.choice(['GAIN', 'LOSS'])
        rd_block = rd.randint(1, 2)
        rd_table = rd.randint(1, 20)
        rd_decision = rd.randint(1, 21)

        # todo: no self here
        # Player Decision
        player_decision = globals()[f"B{rd_block}_{rd_typ}{rd_table}_D{rd_decision}"]

        # # Payoff Info
        # self.participant.vars['payoff_rd_typ'] = rd_typ
        # self.participant.vars['payoff_rd_block'] = rd_block
        # self.participant.vars['payoff_rd_table'] = rd_table
        # self.participant.vars['payoff_rd_decision'] = rd_decision
        # self.participant.vars['payoff_player_decision'] = player_decision

        # Payoff Calc
        if player_decision == 1:
            # Option A
            urn = []
            for _ in np.arange(0, getPayoffOptionA(rd_block, rd_table, rd_typ)['p'] * 100, 1):
                urn.append(getPayoffOptionA(rd_block, rd_table, rd_typ)['x1'])
            for _ in np.arange(0, (1-getPayoffOptionA(rd_block, rd_table, rd_typ)['p'])*100, 1):
                urn.append(getPayoffOptionA(rd_block, rd_table, rd_typ)['x2'])
            rd.shuffle(urn)
            # todo: no self.payoff here, instead return
            return c(
                Constants.basic_fee + (urn[rd.randint(0, 99)]/Constants.multiplier)
            )
            # self.participant.vars['payoff'] = self.payoff
        else:
            # Option B
            # todo: no self.payoff here, instead return
            return c(
                Constants.basic_fee + (getPayoffOptionB(rd_block, rd_table, rd_decision, rd_typ)/Constants.multiplier)
            )
            # self.participant.vars['payoff'] = self.payoff


# TESTING
test = []
for i in np.arange(1, 1000):
    test.append(getPayoff())
print(test)
print(max(test), min(test))

